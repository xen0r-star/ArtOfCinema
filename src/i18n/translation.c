// ⚠️ Créer un « changeLanguage » CORRECT
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
unsigned long hash(const char* str){    // Transformation de key en hash pour stockage rapide table O(1)
...
void addTranslation(TranslationTable* translationTblPrev, const char* key, const char* value){  // Ajout d'une traduction quelconques
    if(!key || !value) return;
