...
void showLoginPage() {
    ...
    if (strcmp(user.email, "") == 0) {
        ...
        createButton(ALIGN_CENTER, rows - 7, 18, PRIMARY_COLOR, _T("login.btn.nxt"), validEmail);
        ...
        if (emailExiste(user.email) == STATUS_FOUND) {
            ...
            createButton(ALIGN_CENTER, rows - 7, 18, PRIMARY_COLOR, _T("login.btn.conn"), signIn);
            ...
            createButton(ALIGN_CENTER, rows - 7, 21, PRIMARY_COLOR, _T("login.btn.crt.acc.next"), nextStepSignUp);
        }
        ...
        switch (user.role) {
            ...
            case ROLE_ERROR:
                ...
                createButton(ALIGN_CENTER, rows - 7, 21, SECONDARY_COLOR, _T("login.acc.conn.visual.txt.err.bck"), backToLogin);
                break;
            ...
            case ROLE_NONE:
                ...
                createButton(ALIGN_CENTER, rows - 7, 21, PRIMARY_COLOR, _T("login.crt.acc.btn"), signUp);
                break;
        }
    }
}