...
static void drawLoadingBar(int progressValue) {
    ...
    // Partie remplie
    cursor(startX, y);
    if (progressWidth <= 25)        setBackground((ColorRGB){197, 15, 31});
    else if (progressWidth <= 50)   setBackground((ColorRGB){217, 156, 0});
    else                            setBackground((ColorRGB){19, 161, 14});
...
void showLoadingScreen() {
    ...
    for (int i = 2; i <= 100; i += 2) {
        // Texte de chargement
        createText(ALIGN_CENTER, rows / 2, _T("loading"), PRIMARY_COLOR);
